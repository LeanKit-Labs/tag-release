...
`{
	...
	describe( "gitRebaseUpstreamDevelopWithConflictFlag", () => {
		it( "should call `git.rebaseUpstreamDevelop`", () => {
			git.rebaseUpstreamDevelop = jest.fn( () => Promise.resolve( { conflict: false } ) );
	...
	describe( "getLocalChanges", () => {
		it( "should set state with localChanges", () => {
			...
			expect( state.cr.localChanges ).toEqual( { "my-perfect-package": "1.1.1-something.3", "my-worst-package": "2.4.1-overwatch.1" } );
		} );
	} );
	...
	describe( "findConflictedPackageJSONChunks", () => {
		it( "should set state with conflicts", () => {
			...
			expect( state.cr.chunks ).toEqual( {
				"		\"@lk/some-package\": \"2.5.0\",": [ "		\"@lk/my-package\": \"14.14.0\",", "		\"@lk/my-other-package\": \"2.0.0\"," ]
			} );
			expect( state.cr.newLines ).toEqual( [ "{", "	\"devDependencies\": {", "		\"@lk/some-package\": \"2.5.0\",", "		\"@lk/some-other-package\": \"1.3.0\",", "	}", "}" ] );
...
`{
	...
	describe( "resolveChunkConflicts", () => {
		it( "should update chunks with localChanges if locally changed packages are the ones in conflict", () => {
			state = {
				...
				cr: {
					chunks: {
						"		\"@lk/some-package\": \"2.5.0\",": [ "		\"@lk/my-perfect-package\": \"1.2.0\"," ]
					},
					localChanges: {
			...
			expect( state.cr ).toHaveProperty( "chunks" );
			expect( state.cr.chunks ).toEqual( {
				"		\"@lk/some-package\": \"2.5.0\",": [ "		\"@lk/my-perfect-package\": \"1.1.1-something.3\"," ]
			} );
		} );
		...
		it( "should use HEAD changes with conflicted package in chunk isn't a pre-release", () => {
			...
			expect( logger.log ).toHaveBeenCalledWith( "You had a local change of 11.2.0 for my-package, but we used HEAD's version of 11.2.0" );
		} );
		...
		it( "should use undefined when version doesn't match regex in conflict chunk", () => {
			state = {
				...
				cr: {
					chunks: {
						"		\"@lk/some-package\": \"2.5.0\",": [ "		\"@lk/my-package\": \"this shouldn't match\"," ]
					},
					localChanges: {
						"my-package": "11.2.0"
					}
			...
			expect( logger.log ).toHaveBeenCalledTimes( 1 );
			expect( logger.log ).toHaveBeenCalledWith( "You had a local change of 11.2.0 for my-package, but we used HEAD's version of undefined" );
		} );
	} );
		...
		it( "should call util.writeFile with appropriate args", () => {
			state = {
				cr: {
					...
					chunks: {
						"		\"@lk/some-package\": \"2.5.0\",": [ "		\"@lk/my-package\": \"14.14.2-filterror.0\"," ]
					},
					newLines: [ "{", "	\"devDependencies\": {", "		\"@lk/some-package\": \"2.5.0\",", "		\"@lk/some-other-package\": \"1.3.0\",", "	}", "}" ]
				},
				configPath: "./package.json"
			};
			run.writeChunksToPackageJSON( state );
			expect( util.writeFile ).toHaveBeenCalledTimes( 1 );
			expect( util.writeFile ).toHaveBeenCalledWith( "./package.json", `{
	\"devDependencies\": {
		\"@lk/some-package\": \"2.5.0\",
		\"@lk/my-package\": \"14.14.2-filterror.0\",
		\"@lk/some-other-package\": \"1.3.0\",
	...
	describe( "resolvePackageJSONConflicts", () => {
		it( "should write to package.json with corrected conflicted chunks and set state accordingly if conflict is true", () => {
			...
			expect( state.cr ).toHaveProperty( "newLines" );
			expect( state.cr ).toHaveProperty( "contents" );
			expect( state.cr.chunks ).toEqual( { "		\"@lk/some-package\": \"2.5.0\",": [ "		\"@lk/my-package\": \"14.14.2-filterror.0\",", "		\"@lk/my-other-package\": \"2.0.0\"," ] } );
